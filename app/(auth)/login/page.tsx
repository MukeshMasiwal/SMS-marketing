...
export default function LoginPage() {
  ...
  const handleDemoLogin = async (role: "USER" | "ADMIN") => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error("Demo account is unavailable. Please run the seed script first.");
      }

      router.push(destination);
      router.refresh();
    } catch (err: any) {
      setError(err.message || "Demo account is unavailable. Please run the seed script first.");
    } finally {
      setLoading(false);
      setDemoLoading(null);
    }
